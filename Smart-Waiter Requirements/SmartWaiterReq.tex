...
\begin{itemize}
  ...
  \item 	Beta Testers
  \item Prospective manager/restaurant owners \ds{Why do you have dots here? If there are more stakeholders you can include, include them. Otherwise, just end the list}
\end{itemize}
...
\begin{itemize}
 ...
 \ds{Why do they need to frequent restaurants at least once a month to qualify for this app? Couldn't anyone use it regardless of how often they go out?}
\end{itemize}
...
\newline
\textit{User Role}\newline
These users will use the application to see how well it fits into their business requirements. These users will also demands features and enhancements.\ds{Wouldn't they be doing something different than just what the customers can do?}\newline\newline
\textit{Subject Matter Experience}\newline
...
\textit{Fit criterion}\newline
The application must be approved \ds{What does ``approved complaint" mean? Did you mean ``compliant"?} from the developers and testers. This will involve thorough testing on the Android platform. \ds{This paragraph is awkward}\newline\newline
...
\textit{Fit criterion}\newline
The functionality must be approved \ds{``Approved complaint" again} by the developer and testers. This will require approving the code base and the end-to-end functionality.\newline\newline
...
The users will be using this application in a restaurant. Typically, this work environment would be loud and distracting. Thus, we plan on making the UI of the application visually attractive, where the user does not need to read much. Specifically, the UI will use larger fonts, pictures and a design which is intuitive and straight forward to use.
...
\textbf{\textit{Fact 1}}\newline
Stripe API used for payment processing is payment card industry (PCI) compliant. \ds{``Compliant"}
\newline
...
\textit{Fit Criterion:}\newline 
A user is browsing the menu and adds a chicken entr\'{e}e \ds{Should be ``entr\'{e}e"}
...
\textit{Fit Criterion:}\newline 
A user is browsing the menu and adds an entree to his order, the total is calculated immediately afterwards.  If they add
...
\textit{Fit Criterion:}\newline 
User should be capable of placing their entire order, including special instructions through the application. Help from restaurant personnel should not be required. \ds{Not a fit criterion}
\newline
...
Increasing number of participating restaurants may induce database upgrade in terms of efficiency in querying. There is one central database for all restaurants.
\ds{Is there one central database for all restaurants?
Where are the database specifications?}

\subsubsection{Longevity Requirements}
App is relies on the restaurants business life. \ds{What does that mean?} For example, if restaurant decides to shutdown, the app will no longer serve that restaurant.\ds{Grammar issues with that sentence}
...
\begin{itemize}
\item Amount of data to be queried is unknown as it is \ds{``it's" is the correct spelling, but don't use contractions. ``it is"} specific to each restaurant.
\item Amount of concurrent clients requesting data is variable. Will affect \ds{affect. I will not be pointing out any more spelling errors, please find them on your own.} efficiency
\end{itemize}
